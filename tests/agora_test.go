...
func TestPatient(t *testing.T) {
	...
	patients, err := project.GetPatients()
	if err != nil {
		t.Errorf("cannot get the patients: %s", err.Error())
	} else if patients == nil {
		t.Errorf("patients is empty")
	}

	patientId := patients[0].ID
	patient, err := agora.GetPatient(patientId)
	if err != nil {
		t.Errorf("cannot get the patient: %s", err.Error())
	}
	assert.Equal(t, patient.ID, patientId, "patient id is not the same")
}
...
func TestStudy(t *testing.T) {
	...
	studies, err := project.GetStudies()
	if err != nil {
		t.Errorf("cannot get the studies: %s", err.Error())
	} else if studies == nil {
		t.Errorf("studies is empty")
	}

	studyID := studies[0].ID
	study, err := agora.GetStudy(studyID)
	if err != nil {
		t.Errorf("cannot get the study: %s", err.Error())
	}
	assert.Equal(t, study.ID, studyID, "study id is not the same")
}
